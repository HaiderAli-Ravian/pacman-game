...
export default function PlayGame() {
  const [pacmanPosition, setPacmanPosition] = useState<Position>(
    game.pacMan.getPosition()
  );
  const [points, setPoints] = useState<number>(game.points);
  const [pacManDirection, setPacManDirection] = useState<DirectionsType>(
            ...
            <div className="flex justify-center gap-5 py-2">
              {game.livesRemaining &&
                [...Array(game.livesRemaining)].map((el, ind) => (
                  <img
                    key={ind}
                    src="pacman/pacman-2.png"
                    className="w-[30px] h-[30px]"
                  />
                ))}
            </div>
            <div className="flex justify-center py-2">
              <p className="text-white font-mono font-extrabold text-xl">
                SCORE: {points}
              </p>
            </div>
          </div>
            ...
            <>
              ...
              {game.status === "GAME_OVER" && (
                <div className=" absolute p-8 bg-gray-800 bg-opacity-80 w-full h-full flex justify-center items-center flex-col gap-3">
                  <p className="animate-pulse text-5xl font-extrabold text-red-500">
                    GAME OVER
                  </p>
